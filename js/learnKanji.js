...
for(var i = 0; i<kanjiData.length; i++){
    ...
    $('#cards').append("<div class='card col s2 offset-s1'><div class='card-image waves-effect waves-block waves-light'><img class='activator' src='"+ kanjiData[i].image +"'></div><div class='card-content'><span class='card-title activator grey-text text-darken-4'><h6 class='truncate'>"+ kanjiData[i].onyomi +"</h6><i class='material-icons right'>more_vert</i></span></div><div class='card-reveal'><span class='card-title grey-text text-darken-4'><i class='material-icons'>close</i></span><h6><b>KUNYOMI</b><br/>"+ kanjiData[i].kunyomi +"</h6><p><b>MEANING</b><br/>"+ kanjiData[i].meaning +"</p><p><b>MNEMONIC</b><br/>"+ kanjiData[i].mnemonic +"</p></div></div>");
    if(i % 4 == 3)
    {
    	$('#cards').append("</div>")
    }
